...
コンポジット映像出力付きのFPV映像受信機と、コンポジット映像用の映像分割機を組み合わせて、簡易的なマルチビューアシステムを構築できます。

PCを使用せず、とにかく安く簡単に4分割画面が出せて、表示の自由度や画質に関しては多くを望まない、という点を重視しています。
...
| 品名 | 役割 | 参考価格 |
| :-- | :-- | --: |
| [ACアダプター 12V 1A](https://www.marutsu.co.jp/pc/i/593895/) | 電源の供給 | 850円 |
...
| [JSTオス、メスプラグコネクタケーブル × 5ペア](https://www.banggood.com/ja/5-Pairs-200mm-20cm-JST-Male-And-Female-Plug-Connector-Cable-ESC-Extension-Cable-p-1125803.html) | 受信機への電源接続 | 180円 |
...
参考価格の合計は15,170円(税込、送料別)となります。

JSTコネクタはオスを3本使用します。JSTコネクタを使用することで、RC832付属の電源ケーブルを切断加工することなく、電源分岐ターミナルへの着脱が可能となります。
...
コンポジットからHDMIへ変換する機器も用意します。これはPAL/NTSCの問題に対応するためのものです。その目的から、必ずしも出力側がHDMIである必要はありませんが、ディスプレイとの接続性や、HDMIから電源を供給できる点も考慮に入れて、選択しています。

### 1.2. PAL/NTSCの問題

ドローンのVTXから送信される映像の方式は、PALだったりNTSCだったりまちまちです。VTXの設定で切り替えられ場合もあれば、どちらかに固定されていて動かせない場合もありそうです。これをRC832で受信すると、おそらく入力側と出力側の映像方式が同じとなります。つまり、PALで受信すればPALで出力、NTSCで受信すればNTSCで出力されるものと予想されます。

映像分割機AV-400の入力は、PALとNTSCの両方に対応しており、入力端子1の映像方式が出力側に引き継がれるようです。また、入力側にPALとNTSCが混在した場合にも、映像の分割表示には(おそらく動作保証外ですが)大きな支障はないようです(**※要検証**)。先ほど説明したように、AV-400の出力側の映像方式は固定ではありません。つまり、PALだったりNTSCだったり環境によってまちまちです。

一方で、日本で売られているテレビやレコーダーのコンポジット入力は、NTSCにしか対応していない場合が多いと考えられます。そのような環境で、無理矢理PALの映像を入力すると、白黒になったり、垂直方向に映像が流れたりするようです。これを解決するために、最終段でPALからNTSCの変換を行います。今回は、PALとNTSCに両対応している、コンポジット->HDMI変換機で代用しています。
