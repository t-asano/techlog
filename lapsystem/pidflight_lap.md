...
結果としては、ドローンがどこにいてもRSSIが高めの値を示しており、thresholdの設定がかなりシビアに感じました。また、計測デバイスとの距離が遠くても、高度が高ければより高いRSSIを示すこともあり、アンテナの指向性への配慮も必要性も感じました。

それでも、ドローンが計測デバイスの直近を通れば、他の場所より高い値を示すことが確認できたので、thresholdを可能な限り高く設定するべくテストを繰り返しました。しかし、この日のテストでは、どれだけthresholdの設定を煮詰めても、せいぜい1回しかラップが計れないという、不可解な現象に悩まされました。

### ドローン実機でのテスト2回目(2018.2.19)

PIDFlight Lapの動作をじっくりと観察したところ、重大な事に気づきました。「Devices」の画面に「Maximum laps」の設定欄があるのですが、2月17日のテストでは空欄としていました。「空欄であれば何回でもラップが計測できる」と思い込んでいたのですが、どうやら内部的な設定値は「1」となっているようです。

つまり、この状態ではラップを1回だけ計測して終了してしまいます。この仕様に気づかずに「なかなか上手く計測できない」と感じていた可能性が高いです。

![app01](img/pfl_app_c01.png)

試しに「Maximum laps」に「10」を設定してみたところ、ようやく想定に近い挙動となりました。また、他にもいくつか注意すべき点が分かってきたので、以下にまとめます。

### PIDFlight Lap使用時の注意点
...
- Maximum lapsで設定した回数しか計測されない
	- 空欄だと1が設定されるようなので要注意
	- とにかく繰り返し計測したい場合は大きな値を入れておけば良い
- Minimum lap timeを設定しておくと誤動作を減らせる
...
- PCアプリの使い方は、[スマホアプリのマニュアル](https://www.pidflight.com/download/298/)がある程度参考になる
...
ブースターを内蔵することで長尺に対応した延長ケーブルを使用します。下記の製品では、15mの延長が可能です。0.5mのUSB miniBケーブルとの組み合わせで、問題なく動作しました。(5mのUSBケーブルとの組み合わせは未検証です)

- [Simble USB 延長線 15m USB2.0 (PC-643) (参考価格:1,880円)](https://www.amazon.co.jp/dp/B008988WBE)

### LANケーブルで延長するためのアダプタ

こちらは延長区間をLANケーブルで補います。下記の製品では、20mのLANケーブル、0.5mのUSB miniBケーブルとの組み合わせで、概ね問題なく動作しました。(20m超のLANケーブルや、5mのUSBケーブルとの組み合わせは未検証です)
...
[公式サイト](https://www.pidflight.com/pidflight-lap/multipilot/)によれば回路はシンプルで、物理デバイスを直列に繋げば良く、PCとの接続は1箇所で済むようです。有線で接続する場合、ArduinoのUSBポートは使用せず、ArduinoのTx/Rxピンに接続することになります。そのためには、USBシリアル変換モジュールの追加が必要と思われます。(近日検証予定)
